...
function Simpson() {
  const [showSimpsonOnly, setShowSimpsonOnly] = useState(false);
  ...
  function handleShowSimpsonOnlyClick() {
    setShowSimpsonOnly(!showSimpsonOnly);
  }

  return (
    <div>
      <button onClick={handleShowSimpsonOnlyClick}>Simpson family only</button>
      {quoteList
      .filter((quote) => !showSimpsonOnly || quote.character.includes('Simpson'))
      .map((quote, index) => (
